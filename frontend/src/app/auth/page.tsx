      ...
      <div className="w-full max-w-md space-y-8">
        ...
        <div className="w-full bg-white rounded-lg shadow-lg border border-gray-200">
          ...
          <div className="flex w-full border-b border-gray-200">
            <div 
              className={`flex-1 py-2 text-center cursor-pointer border-b-2 ${activeTab === 'user' ? 'border-blue-600 text-blue-600' : 'border-gray-200 text-gray-500'}`}
              onClick={() => setActiveTab("user")}
            >
              일반 로그인
            </div>
            <div 
              className={`flex-1 py-2 text-center cursor-pointer border-b-2 ${activeTab === 'admin' ? 'border-blue-600 text-blue-600' : 'border-gray-200 text-gray-500'}`}
              ...
              <form onSubmit={(e) => handleLogin(e, false)} className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label htmlFor="user-email" className="text-sm font-medium block">
                      이메일
                    </label>
                    <input
                      id="user-email"
                  ...
                  <div className="space-y-1">
                    <label htmlFor="user-password" className="text-sm font-medium block">
                      비밀번호
                    </label>
                    <input 
                      id="user-password" 
                      ...
                      className="w-full p-2 border border-gray-300 rounded focus:border-blue-500"
                      value={password.user}
                      onChange={(e) => setPassword(prev => ({ ...prev, user: e.target.value }))}
                      required 
                    />
                  </div>
                </div>
          ...
          {activeTab === "admin" && (
            <div className="p-6">
              <form onSubmit={(e) => handleLogin(e, true)} className="space-y-4">
                ...
                <div className="space-y-4">
                  <div className="space-y-1">
                    <label htmlFor="admin-email" className="text-sm font-medium block">
                      이메일
                    </label>
                    <input
                      id="admin-email"
                      ...
                      value={email.admin}
                      onChange={(e) => setEmail(prev => ({ ...prev, admin: e.target.value }))}
                      required
                    />
                  </div>
                  <div className="space-y-1">
                    <label htmlFor="admin-password" className="text-sm font-medium block">
                      비밀번호
                    </label>
                    <input 
                      id="admin-password" 
                      ...
                      onChange={(e) => setPassword(prev => ({ ...prev, admin: e.target.value }))}
                      required 
                    />
                  </div>
                </div>

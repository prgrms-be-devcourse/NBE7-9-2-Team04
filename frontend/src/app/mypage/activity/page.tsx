...
export default function MyActivityPage() {
  ...
  const fetchUserComments = async (id: number) => {
    try {
      const res = await fetchApi(`/api/v1/users/${id}/comments?page=${commentPage}`);
      if (res.status === "OK") {
  ...
  const fetchUserQuestions = async (id: number) => {
    try {
      const res = await fetchApi(`/api/v1/users/${id}/questions?page=${questionPage}`);
      if (res.status === "OK") {
            ...
            <div className="space-y-3">
              {userPosts.length === 0 ? (
                ...
                userPosts.map((post) => (
                  <div
                    key={post.postId}
                    onClick={() => router.replace(`/recruitment/${post.postId}`)}

...
export default function QnAPage() {
  ...
  const getCategoryColor = (category: string) => {
    const colorMap: Record<string, string> = {
      계정: "bg-indigo-50 text-indigo-700 border-indigo-200", // Indigo
      결제: "bg-blue-50 text-blue-700 border-blue-200", // Blue
      시스템: "bg-slate-50 text-slate-700 border-slate-200", // Slate
      모집: "bg-purple-50 text-purple-700 border-purple-200", // Purple
      제안: "bg-emerald-50 text-emerald-700 border-emerald-200", // Emerald
      기타: "bg-amber-50 text-amber-700 border-amber-200", // Amber
    };
    ...
    const fetchQnaList = async () => {
      try {
        ...
        if (selectedCategory !== "전체") {
          ...
          url = `/api/v1/qna/category/${map[selectedCategory]}`;
        }

        const apiResponse = await fetchApi(url, { method: "GET" });
    ...
    <div className="min-h-screen px-4 py-10 flex flex-col items-center bg-gray-50">
      <div className="w-full max-w-5xl space-y-10">
        ...
        <div className="space-y-4">
          ...
          {!isLoading && errorMsg && (
            <div className="bg-red-50 text-red-600 border border-red-200 rounded-md text-center py-6">
              {errorMsg}
            </div>
          )}
